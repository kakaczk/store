package com.sundiromall.order.service;
...
import com.sundiromall.order.bean.SundiroResult;
import com.sundiromall.order.dao.IOrder;
import com.sundiromall.order.pojo.Order;
import com.sundiromall.order.pojo.PageResult;
import com.sundiromall.order.pojo.ResultMsg;
import com.sundiromall.order.util.ValidateUtil;
...
//    private RabbitTemplate rabbitTemplate;
    ...
    public SundiroResult createOrder(String json) {
        Order order = null;
        try {
            ...
            return SundiroResult.build(400, "请求参数有误!");
        }
...
//            this.rabbitTemplate.convertAndSend(objectMapper.writeValueAsString(msg));
            ...
            return SundiroResult.ok(orderId);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return SundiroResult.build(400, "保存订单失败!");
    }
